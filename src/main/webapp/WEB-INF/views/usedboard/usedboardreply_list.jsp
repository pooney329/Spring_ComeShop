	...
	replyadd = function(th){
		...
		button.click(function(){
			...
			$.ajax({
				type:"post",
				data: data,
				url : "${path}/usedboard/usedreplyadd",
...
<body>
	...
	<ol style="list-style:none; height:100%;" >
		...
		<c:set var="name" value="${row.uindent*29}"/>
		...
		<li style="padding-left:${name}px; width:100%; height:100% ;display:inline-block;">
			...
			<c:if test="${row.replyer!=sessionScope.loginuser.uid}"><button onclick="replyadd(this)" class="btn btn-primary" data-ugroup="${row.ugroup}" data-ustep="${row.ustep}" data-uindent="${row.uindent}">댓글</button></c:if>
			<c:if test="${row.replyer==sessionScope.loginuser.uid}">
				<button class="btn btn-primary">수정</button>
				<button onclick="replydelete(this);" data-id="${row.urno}" class="btn btn-primary">삭제</button>
			</c:if>
			</div>
				</c:if>
				
				<c:if test="${row.udel==1}">
					<div class="col-sm-10" style="height:100px; padding:30px; font-size:20px;">삭제된 댓글입니다.
