package com.example.Computer.shopapi;
...
public class OpenApi11st {
	OpenApiDTO[] api11stdtolist;
	
	public OpenApi11st() {
		try {

			// parsing할 url 지정(API 키 포함해서)
			String text = URLEncoder.encode("삼성전자 DDR4 8G PC4-21300", "UTF-8");
			String url = "http://openapi.11st.co.kr/openapi/OpenApiService.tmall?key=9f970fb1c7502e1184f90c361c3b9277&apiCode=ProductSearch&keyword="
					+ text + "&sortCd=A";
			...
			for (int i = 0; i < n1.getLength(); i++) {
				...
				list.add(new OpenApiDTO(price, delivery, detailpageurl, mallname));
			...
			api11stdtolist = new OpenApiDTO[list.size()];
	...
	public OpenApiDTO[] getApi11stdtolist() {
		return api11stdtolist;
	}

	public void setApi11stdtolist(OpenApiDTO[] api11stdtolist) {
		this.api11stdtolist = api11stdtolist;
	}
